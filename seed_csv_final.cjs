...
async function seed() {
  ...
  for (let i = 0; i < validRows.length; i += batchSize) {
    ...
    for (const row of chunk) {
      ...
      const rowValues = [
        ...
        parseNumber(row['Deposito_Vista_vs_Funding (%)']), // New column!
    ...
    const queryText = `
      ...
      ON CONFLICT (codigo) DO UPDATE SET
        ...
        deposito_vista_funding = EXCLUDED.deposito_vista_funding,
        pcld = EXCLUDED.pcld,
  ...
  const sampleQuery = await client.query(`
    SELECT codigo, nome, ativo_total, carteira_credito, razao_alavancagem, deposito_vista_funding, ib
    FROM emissores_bancarios
    ORDER BY ativo_total DESC
    LIMIT 3;
  `);
  console.log('\nTop 3 banks by Ativo Total (should be ITAU, BB, CAIXA):');
